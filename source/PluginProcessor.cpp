...
double quantizeCaptureQuarters (const double quarters)
{
    return std::round (quarters / PluginProcessor::stepTimingMultiplierQuarterStep)
           * PluginProcessor::stepTimingMultiplierQuarterStep;
}
...
void PluginProcessor::resetPlaybackMidiState()
{
    ...
    resolvePendingAudioLoopBraceRangeForStoppedPlayback();
    ...
    switch (command.type)
    {
        ...
        case SequencerCommand::Type::SetLoopBraceEnabled:
            ...
            else
            {
                ...
                if (pendingAudioLoopBraceRangePatternSlot == patternSlot)
        ...
        case SequencerCommand::Type::SetLoopBraceStart:
        {
            if (patternSlot == audioActivePatternSlot
                ...
                && pattern.loopBrace.enabled != 0)
            {
                const auto pendingEnd =
                    pendingAudioLoopBraceRangePatternSlot == patternSlot
                        ? pendingAudioLoopBraceEndQuarters
                        : pattern.loopBrace.endQuarters;
                requestAudioLoopBraceRange (patternSlot, command.doubleValue, pendingEnd);
            }
            else
            {
        ...
        case SequencerCommand::Type::SetLoopBraceEnd:
        {
                ...
                && pattern.loopBrace.enabled != 0)
            {
                const auto pendingStart =
                    pendingAudioLoopBraceRangePatternSlot == patternSlot
                        ? pendingAudioLoopBraceStartQuarters
                        : pattern.loopBrace.startQuarters;
                requestAudioLoopBraceRange (patternSlot, pendingStart, command.doubleValue);
            }
            else
            {
                pattern.loopBrace.endQuarters =
...
void PluginProcessor::requestAudioLoopBraceRange (const int patternSlot,
                                                  const double startQuarters,
                                                  const double endQuarters)
{
    const auto slot = clampPatternSlot (patternSlot);
    const auto loopStart = clampLoopBraceStart (startQuarters, endQuarters);
    const auto loopEnd = clampLoopBraceEnd (endQuarters, loopStart);

    pendingAudioLoopBraceRangePatternSlot = slot;
    pendingAudioLoopBraceStartQuarters = loopStart;
    pendingAudioLoopBraceEndQuarters = loopEnd;
}

void PluginProcessor::clearPendingAudioLoopBraceRange()
{
    pendingAudioLoopBraceRangePatternSlot = -1;
    pendingAudioLoopBraceStartQuarters = defaultLoopBraceStartQuarters;
    pendingAudioLoopBraceEndQuarters = defaultLoopBraceEndQuarters;
}

void PluginProcessor::applyAudioLoopBraceRange (const double reanchorTransportPpq)
{
    const auto slot = pendingAudioLoopBraceRangePatternSlot;
    const auto loopStart = pendingAudioLoopBraceStartQuarters;
    const auto loopEnd = pendingAudioLoopBraceEndQuarters;
    clearPendingAudioLoopBraceRange();
    ...
    loopBrace.startQuarters = clampLoopBraceStart (loopStart, loopEnd);
    loopBrace.endQuarters = clampLoopBraceEnd (loopEnd, loopBrace.startQuarters);

    if (slot != audioActivePatternSlot || loopBrace.enabled == 0)
        return;
...
void PluginProcessor::resolvePendingAudioLoopBraceRangeForStoppedPlayback()
{
    if (pendingAudioLoopBraceRangePatternSlot < 0)
        return;

    applyAudioLoopBraceRange (-1.0);
    clearLoopScheduleAnchor();
}
...
{
    pendingSchedulePhaseReset.store (1, std::memory_order_release);
}

void PluginProcessor::applySchedulePhaseReset (const double reanchorTransportPpq)
{
    pendingSchedulePhaseReset.store (0, std::memory_order_release);
...
{
    const auto clamped = clampStandaloneTempoBpm (bpm);
    standaloneTempoBpm.store (clamped, std::memory_order_relaxed);
    latestRecordingTempoBpm.store (clamped, std::memory_order_relaxed);
}
...
int PluginProcessor::clampRecordCycleLengthPulses (const int pulses)
{
    return juce::jlimit (minRecordCycleLengthPulses,
                         maxRecordCycleLengthPulses,
                         pulses);
}

int PluginProcessor::defaultRecordCycleLengthPulsesForRow (const int patternSlot,
                                                           const int row) const
{
    if (row < 0 || row >= phraseRowCount)
        return defaultRecordCycleLengthPulses;

    const auto pulse = pulseQuartersForIndex (pulseIndex.load (std::memory_order_relaxed));
    const auto& steps =
        modelPattern (patternSlot).sequencer.rows[static_cast<size_t> (row)];

    if (pulse <= 0.0 || steps.cycleLengthQuarters <= 0.0)
        return defaultRecordCycleLengthPulses;

    return clampRecordCycleLengthPulses (
        static_cast<int> (std::lround (steps.cycleLengthQuarters / pulse)));
}

int PluginProcessor::resolvedRecordCycleLengthPulses (const int patternSlot,
                                                      const int row) const
{
    if (row < 0 || row >= phraseRowCount)
        return defaultRecordCycleLengthPulses;
    ...
    const auto stored =
        modelPattern (patternSlot).recordCycleLengthPulses[static_cast<size_t> (row)];

    if (stored >= minRecordCycleLengthPulses)
        return clampRecordCycleLengthPulses (stored);

    return defaultRecordCycleLengthPulsesForRow (patternSlot, row);
}

int PluginProcessor::getPatternRecordCycleLengthPulses (const int patternSlot,
                                                        const int row) const
{
    return resolvedRecordCycleLengthPulses (patternSlot, row);
}

void PluginProcessor::setPatternRecordCycleLengthPulses (const int patternSlot,
                                                         const int row,
...
double PluginProcessor::recordingLoopLengthQuarters() const
{
    const auto pulse = pulseQuartersForIndex (pulseIndex.load (std::memory_order_relaxed));
    return pulse
           * static_cast<double> (
               clampRecordCycleLengthPulses (
                   recordingCycleLengthPulses.load (std::memory_order_acquire)));
}

double PluginProcessor::recordingLoopPhaseForPpq (const double ppq) const
{
    const auto loopLength = recordingLoopLengthQuarters();

    if (loopLength <= 0.0 || ppq < 0.0)
        return 0.0;

    auto phase = std::fmod (ppq, loopLength);
...
double PluginProcessor::recordingCurrentTransportPpq() const
{
    const auto latest = latestRecordingTransportPpq.load (std::memory_order_acquire);

    if (latest >= 0.0)
        return latest;

    const auto playback = currentPlaybackPpq.load (std::memory_order_relaxed);
...
double PluginProcessor::getPhraseRowRecordingProgress() const
{
    ...
    return juce::jlimit (0.0,
                         1.0,
                         recordingLoopPhaseForPpq (recordingCurrentTransportPpq())
                             / loopLength);
}
                                             ...
                                             const RecordingMode requestedMode)
{
    ...
    const auto previouslyActiveNote =
        recordingActiveNote.exchange (-1, std::memory_order_acq_rel);

    if (previouslyActiveNote >= 0)
    {
        const auto previousRow = recordingRow.load (std::memory_order_acquire);

        if (previousRow >= 0 && previousRow < phraseRowCount)
        {
            enqueueRecordingLoopSpan (
                previousRow,
                previouslyActiveNote,
                recordingActiveVelocity.load (std::memory_order_acquire),
                recordingActiveStartQuarters.load (std::memory_order_acquire),
                recordingLoopPhaseForPpq (recordingCurrentTransportPpq()));
        }
    }
...
void PluginProcessor::setPhraseRowRecordingCycleLength (const int cycleLengthPulses)
{
    ...
    const auto clamped = clampRecordCycleLengthPulses (cycleLengthPulses);
    setPatternRecordCycleLengthPulses (getViewPatternSlot(), row, clamped);
    recordingCycleLengthPulses.store (clamped, std::memory_order_release);
}

int PluginProcessor::getPhraseRowRecordingCycleLength() const
{
    return clampRecordCycleLengthPulses (
        recordingCycleLengthPulses.load (std::memory_order_acquire));
}
...
void PluginProcessor::resetPhraseRowRecordingCapture()
{
    ...
    recordingActiveVelocity.store (defaultStepVelocity, std::memory_order_release);
    recordingActiveStartQuarters.store (0.0, std::memory_order_release);

    for (auto& event : recordingCaptureEvents)
    {
...
{
    const auto quantized = juce::jmax (
        stepTimingMultiplierMin,
        quantizeCaptureQuarters (quarters));
    const auto index = static_cast<int> (std::round (
        (quantized - stepTimingMultiplierMin) / stepTimingMultiplierQuarterStep));

    return juce::jlimit (0, stepTimingMultiplierCount - 1, index);
}

void PluginProcessor::enqueueRecordingLoopSpan (const int row,
                                                const int midiNote,
                                                const int velocity,
                                                const double startQuarters,
                                                const double endQuarters)
{
    ...
    const auto write = recordingLoopSpanQueueWrite.load (std::memory_order_relaxed);
    const auto read = recordingLoopSpanQueueRead.load (std::memory_order_acquire);

    if (write - read >= recordingLoopSpanQueueCapacity)
        return;

    const auto index = static_cast<size_t> (write % recordingLoopSpanQueueCapacity);
    recordingLoopSpanRows[index] = row;
    recordingLoopSpanNotes[index] = juce::jlimit (0, 127, midiNote);
    recordingLoopSpanVelocities[index] = juce::jlimit (1, 127, velocity);
    recordingLoopSpanStarts[index] = startQuarters;
    recordingLoopSpanEnds[index] = endQuarters;
    recordingLoopSpanQueueWrite.store (write + 1, std::memory_order_release);
}

bool PluginProcessor::tryDequeueRecordingLoopSpan (int& rowOut,
                                                   int& midiNoteOut,
                                                   int& velocityOut,
                                                   double& startQuartersOut,
                                                   double& endQuartersOut)
{
    const auto read = recordingLoopSpanQueueRead.load (std::memory_order_relaxed);
    const auto write = recordingLoopSpanQueueWrite.load (std::memory_order_acquire);

    if (read >= write)
        return false;

    const auto index = static_cast<size_t> (read % recordingLoopSpanQueueCapacity);
    rowOut = recordingLoopSpanRows[index];
    midiNoteOut = recordingLoopSpanNotes[index];
    velocityOut = recordingLoopSpanVelocities[index];
    startQuartersOut = recordingLoopSpanStarts[index];
    endQuartersOut = recordingLoopSpanEnds[index];
    recordingLoopSpanQueueRead.store (read + 1, std::memory_order_release);
    return true;
}

void PluginProcessor::capturePhraseRowRecordedLoopEvent (const int midiNote,
                                                         const int velocity,
                                                         const bool noteOn,
                                                         const double eventPpq)
{
    ...
    latestRecordingTransportPpq.store (eventPpq, std::memory_order_release);

    const auto clamped = juce::jlimit (0, 127, midiNote);
    const auto phase = recordingLoopPhaseForPpq (eventPpq);
    ...
    if (noteOn)
    {
        ...
        const auto previousNote =
            recordingActiveNote.exchange (clamped, std::memory_order_acq_rel);
        ...
        recordingActiveVelocity.store (juce::jlimit (1, 127, velocity),
                                       std::memory_order_release);
        recordingActiveStartQuarters.store (phase, std::memory_order_release);
        recordingAwaitingFirstNote.store (0, std::memory_order_release);
        enqueueRecordedNote (clamped);
        return;
    }

    auto activeNote = recordingActiveNote.load (std::memory_order_acquire);

    if (activeNote != clamped)
        return;

    if (recordingActiveNote.compare_exchange_strong (
            activeNote,
            -1,
            std::memory_order_acq_rel))
    {
...
void PluginProcessor::applyRecordingLoopSpanToRow (const int row,
                                                   const int midiNote,
                                                   const int velocity,
                                                   const double startQuarters,
                                                   const double endQuarters)
{
    ...
    const auto cycleLength = recordingLoopLengthQuarters();

    if (pulse <= 0.0 || grid <= 0.0 || cycleLength <= 0.0)
        return;

    const auto quantize = [grid, cycleLength] (const double value)
    {
        auto quantized = std::round (value / grid) * grid;

        while (quantized < 0.0)
            quantized += cycleLength;

        while (quantized >= cycleLength)
            quantized -= cycleLength;
    ...
    std::array<LoopEvent, maxPhraseStepsPerRow * 2 + 4> events {};
    auto eventCount = 0;

    const auto appendEvent = [&events, &eventCount, cycleLength] (
                                 const double eventStart,
                                 const double eventEnd,
                                 const int note,
                                 const int eventVelocity)
    {
        if (eventCount >= static_cast<int> (events.size()))
            return;

        const auto clampedStart = juce::jlimit (0.0, cycleLength, eventStart);
        const auto clampedEnd = juce::jlimit (0.0, cycleLength, eventEnd);

        if (clampedEnd <= clampedStart + epsilon)
            return;

        auto& target = events[static_cast<size_t> (eventCount++)];
        target.start = clampedStart;
        target.end = clampedEnd;
        target.note = juce::jlimit (0, 127, note);
        target.eventVelocity = juce::jlimit (1, 127, eventVelocity);
    };

    const auto& source = modelRow (row);

    for (int step = 0; step < source.stepCount; ++step)
    {
        const auto index = static_cast<size_t> (step);

        if (source.stepSkipped[index] != 0 || source.stepMuted[index] != 0
            || source.velocity[index] <= 0 || source.durationFraction[index] <= 0.0)
            continue;

        const auto eventStart = source.stepStartQuarters[index];

        if (eventStart >= cycleLength - epsilon)
            continue;

        const auto eventEnd =
            juce::jmin (cycleLength,
                        eventStart + source.stepLengthQuarters[index]
                                         * source.durationFraction[index]);
        appendEvent (eventStart, eventEnd, source.notes[index], source.velocity[index]);
    }

    const auto replaceRange = [&events, &eventCount, &appendEvent, midiNote, velocity] (
                                  const double replaceStart,
                                  const double replaceEnd)
    {
        if (replaceEnd <= replaceStart + epsilon)
            return;

        std::array<LoopEvent, maxPhraseStepsPerRow * 2 + 4> nextEvents {};
        auto nextCount = 0;

        const auto appendNext = [&nextEvents, &nextCount] (const LoopEvent& event)
        {
            if (nextCount < static_cast<int> (nextEvents.size()))
                nextEvents[static_cast<size_t> (nextCount++)] = event;
        };
        ...
        for (int index = 0; index < eventCount; ++index)
        {
            ...
            if (event.end <= replaceStart + epsilon || event.start >= replaceEnd - epsilon)
            {
                appendNext (event);
                continue;
            }

            if (event.start < replaceStart - epsilon)
            {
                auto left = event;
                left.end = replaceStart;
                appendNext (left);
            }

            if (event.end > replaceEnd + epsilon)
            {
                auto right = event;
                right.start = replaceEnd;
                appendNext (right);
            }
        }
        ...
        appendEvent (replaceStart, replaceEnd, midiNote, velocity);
    };

    auto quantizedStart = quantize (startQuarters);
    auto quantizedEnd = quantize (endQuarters);

    if (std::abs (quantizedEnd - quantizedStart) <= epsilon)
    {
        quantizedEnd = quantizedStart + grid;

        if (quantizedEnd >= cycleLength - epsilon)
            quantizedEnd -= cycleLength;
    }

    if (quantizedEnd > quantizedStart)
    {
        replaceRange (quantizedStart, quantizedEnd);
    }
    else
    {
        replaceRange (quantizedStart, cycleLength);
        replaceRange (0.0, quantizedEnd);
    }

    std::sort (events.begin(),
               events.begin() + eventCount,
               [] (const LoopEvent& left, const LoopEvent& right)
               {
                   if (std::abs (left.start - right.start) > 1.0e-9)
    ...
    for (int index = 0; index < eventCount && stepCount < maxPhraseStepsPerRow; ++index)
    {
        ...
        const auto nextStart =
            index + 1 < eventCount
                ? events[static_cast<size_t> (index + 1)].start
                : cycleLength;
        const auto regionEnd = juce::jmax (event.end, nextStart);
        appendRegion (event.note,
                      event.eventVelocity,
                      event.start,
                      juce::jmin (cycleLength, regionEnd),
                      event.end);
        cursor = juce::jmin (cycleLength, regionEnd);
    }
                                                    ...
                                                    const double eventSeconds)
{
    ...
    if (noteOn)
    {
        ...
        if (previousActive >= 0 && previousActive < maxPhraseStepsPerRow)
        {
            auto& previous =
        ...
        auto reservedIndex = -1;
        auto count = recordingCaptureCount.load (std::memory_order_acquire);

        while (count < maxPhraseStepsPerRow)
        {
            if (recordingCaptureCount.compare_exchange_weak (
                    count,
                    count + 1,
                    std::memory_order_acq_rel))
            {
    ...
    {
        auto& event = recordingCaptureEvents[static_cast<size_t> (index)];

        if (event.ready.load (std::memory_order_acquire) == 0
            || event.active.load (std::memory_order_acquire) == 0
        ...
        auto expected = index;
        recordingActiveEventIndex.compare_exchange_strong (
            expected,
            -1,
            std::memory_order_acq_rel);
        break;
...
void PluginProcessor::capturePhraseRowRecordedNoteTransportEvent (const int midiNote,
                                                                  const int velocity,
                                                                  const bool noteOn,
                                                                  const double eventPpq)
{
    capturePhraseRowRecordedLoopEvent (midiNote, velocity, noteOn, eventPpq);
}

void PluginProcessor::capturePhraseRowRecordedNoteOn (const int midiNote,
                                                      const int velocity)
{
    const auto row = recordingRow.load (std::memory_order_acquire);
    ...
    if (row >= 0 && row < phraseRowCount)
    {
        enqueueRecordingKeyboardMidiEvent (
            modelSequencer().midiChannel[static_cast<size_t> (row)],
            midiNote,
            velocity,
            true);
    }
...
{
    const auto row = recordingRow.load (std::memory_order_acquire);
    const auto channel = row >= 0 && row < phraseRowCount
                             ? modelSequencer().midiChannel[static_cast<size_t> (row)]
                             : defaultPhraseRowMidiChannel;

    enqueueRecordingKeyboardMidiEvent (channel,
                                       midiNote,
                                       defaultStepVelocity,
                                       false);
    if (getPhraseRowRecordingMode() == RecordingMode::starter)
...
bool PluginProcessor::finishPhraseRowRecordingCapture (int& rowOut)
{
    ...
    auto drainedRow = rowOut;
    const auto hadUpdates = drainPhraseRowRecordingUpdates (drainedRow);
    resetPhraseRowRecordingCapture();
    recordQueueWrite.store (0, std::memory_order_release);
    recordQueueRead.store (0, std::memory_order_release);
    recordingMode.store (static_cast<int> (RecordingMode::loopOverdub),
...
void PluginProcessor::enqueueRecordingKeyboardMidiEvent (const int channel,
                                                        const int midiNote,
                                                        const int velocity,
                                                        const bool noteOn)
{
    const auto write = recordingKeyboardMidiQueueWrite.load (std::memory_order_relaxed);
    const auto read = recordingKeyboardMidiQueueRead.load (std::memory_order_acquire);

    if (write - read >= recordingKeyboardMidiQueueCapacity)
        return;

    const auto index = static_cast<size_t> (write % recordingKeyboardMidiQueueCapacity);
    recordingKeyboardMidiChannels[index] =
        juce::jlimit (minPhraseRowMidiChannel, maxPhraseRowMidiChannel, channel);
    recordingKeyboardMidiNotes[index] = juce::jlimit (0, 127, midiNote);
    recordingKeyboardMidiVelocities[index] = juce::jlimit (1, 127, velocity);
    recordingKeyboardMidiNoteOns[index] = noteOn ? 1 : 0;
    recordingKeyboardMidiQueueWrite.store (write + 1, std::memory_order_release);
}

bool PluginProcessor::tryDequeueRecordingKeyboardMidiEvent (int& channelOut,
                                                           int& midiNoteOut,
                                                           int& velocityOut,
                                                           bool& noteOnOut)
{
    const auto read = recordingKeyboardMidiQueueRead.load (std::memory_order_relaxed);
    const auto write = recordingKeyboardMidiQueueWrite.load (std::memory_order_acquire);

    if (read >= write)
        return false;

    const auto index = static_cast<size_t> (read % recordingKeyboardMidiQueueCapacity);
    channelOut = recordingKeyboardMidiChannels[index];
    midiNoteOut = recordingKeyboardMidiNotes[index];
    velocityOut = recordingKeyboardMidiVelocities[index];
    noteOnOut = recordingKeyboardMidiNoteOns[index] != 0;
    recordingKeyboardMidiQueueRead.store (read + 1, std::memory_order_release);
    return true;
}

void PluginProcessor::requestRecordingKeyboardMidiFlush()
{
    recordingKeyboardMidiFlushRequested.store (1, std::memory_order_release);
}

void PluginProcessor::drainRecordingKeyboardMidiEvents (juce::MidiBuffer& midiMessages)
{
    if (recordingKeyboardMidiFlushRequested.exchange (0, std::memory_order_acq_rel) != 0)
    {
        for (int channel = minPhraseRowMidiChannel; channel <= maxPhraseRowMidiChannel; ++channel)
        {
            auto& channelNotes =
                activeRecordingKeyboardNoteCounts[static_cast<size_t> (channel - 1)];
    ...
    int channel = defaultPhraseRowMidiChannel;
    int note = defaultStepNote;
    int velocity = defaultStepVelocity;
    auto noteOn = false;

    while (tryDequeueRecordingKeyboardMidiEvent (channel, note, velocity, noteOn))
    {
        const auto clampedChannel =
            juce::jlimit (minPhraseRowMidiChannel, maxPhraseRowMidiChannel, channel);
        const auto clampedNote = juce::jlimit (0, 127, note);
        auto& activeCount =
            activeRecordingKeyboardNoteCounts[static_cast<size_t> (clampedChannel - 1)]
                                             [static_cast<size_t> (clampedNote)];
        ...
        if (activeCount > 0)
        {
            while (activeCount > 0)
            {
                midiMessages.addEvent (juce::MidiMessage::noteOff (clampedChannel, clampedNote), 0);
                --activeCount;
            }
            continue;
        }

        for (int activeChannel = minPhraseRowMidiChannel;
             activeChannel <= maxPhraseRowMidiChannel;
             ++activeChannel)
        {
            auto& channelCount =
                activeRecordingKeyboardNoteCounts[static_cast<size_t> (activeChannel - 1)]
                                                 [static_cast<size_t> (clampedNote)];

            while (channelCount > 0)
            {
                midiMessages.addEvent (juce::MidiMessage::noteOff (activeChannel, clampedNote), 0);
                --channelCount;
            }
        }
    }
...
bool PluginProcessor::drainPhraseRowRecordingUpdates (int& rowOut)
{
    auto updated = false;
    auto row = -1;
    auto note = defaultStepNote;
    auto velocity = defaultStepVelocity;
    auto start = 0.0;
    auto end = 0.0;

    while (tryDequeueRecordingLoopSpan (row, note, velocity, start, end))
    {
        if (row < 0 || row >= phraseRowCount)
            continue;

        applyRecordingLoopSpanToRow (row, note, velocity, start, end);
        rowOut = row;
        updated = true;
    }
...
void PluginProcessor::handleIncomingControlNotes (juce::MidiBuffer& midiMessages,
                                                  const bool transportClockAvailable,
                                                  const double transportPpqStart,
                                                  const double ppqPerSample)
{
    const auto armedRow = recordingRow.load (std::memory_order_acquire);

    const auto noteOnCountAtSample = [&midiMessages] (const int samplePosition)
    {
        auto count = 0;
    ...
    const auto captureEventPpq = [transportPpqStart, ppqPerSample] (const int samplePosition)
    {
    ...
    for (const auto metadata : midiMessages)
    {
        ...
        if (message.isNoteOnOrOff())
        {
            if (armedRow >= 0)
            {
                if (! message.isNoteOn())
                {
                    ...
                    else if (transportClockAvailable)
                    {
                        capturePhraseRowRecordedNoteTransportEvent (
                            ...
                            defaultStepVelocity,
                            false,
                            captureEventPpq (metadata.samplePosition));
                    }
                    else
                    {
                        capturePhraseRowRecordedLoopEvent (
                ...
                const auto countAtSample = noteOnCountAtSample (metadata.samplePosition);
                                    ...
                                    juce::MidiBuffer& midiMessages)
{
    ...
    auto recordingTransportClockAvailable = false;
    auto recordingTransportPpqStart = 0.0;
    auto recordingPpqPerSample = 0.0;

    if (hasStandaloneTransport())
    {
        const auto recordingBpm =
            clampStandaloneTempoBpm (standaloneTempoBpm.load (std::memory_order_relaxed));
        latestRecordingTempoBpm.store (recordingBpm, std::memory_order_relaxed);

        if (isStandaloneTransportPlaying() && recordingBpm > 0.0)
        {
            recordingTransportClockAvailable = true;
            recordingTransportPpqStart =
                standaloneTransportPpqPosition.load (std::memory_order_relaxed);
            recordingPpqPerSample = (recordingBpm / 60.0) / sampleRateHz;
        }
    }
    else if (const auto* playHead = getPlayHead())
    {
        ...
        if (position.hasValue())
        {
            latestRecordingTempoBpm.store (
                juce::jmax (20.0, position->getBpm().orFallback (120.0)),
                std::memory_order_relaxed);

            const auto recordingBpm = position->getBpm().orFallback (120.0);

            if (position->getIsPlaying() && recordingBpm > 0.0)
            {
                recordingTransportClockAvailable = true;
                recordingTransportPpqStart = position->getPpqPosition().orFallback (0.0);
                recordingPpqPerSample = (recordingBpm / 60.0) / sampleRateHz;
            }
        }
    }

    if (recordingTransportClockAvailable)
    {
        latestRecordingTransportPpq.store (
            recordingTransportPpqStart + static_cast<double> (bufferSamples) * recordingPpqPerSample,
            std::memory_order_relaxed);
    }

    handleIncomingControlNotes (midiMessages,
                                recordingTransportClockAvailable,
                                recordingTransportPpqStart,
                                recordingPpqPerSample);
    ...
    while (transportCursor < ppqEnd - epsilon)
    {
        ...
        const auto pendingLoopBraceRange = pendingAudioLoopBraceRangePatternSlot;
        ...
        if (pendingPattern >= 0 || pendingLoop >= 0 || pendingLoopBraceEnable >= 0
            || pendingLoopBraceRange >= 0 || pendingPhaseReset || pendingCombinationModeMask >= 0)
        {
            const auto pulse = pulseQuartersForIndex (pulseIndex.load (std::memory_order_relaxed));
            ...
            if (pendingPattern >= 0 || pendingLoop >= 0 || pendingLoopBraceEnable >= 0
                || pendingLoopBraceRange >= 0 || pendingPhaseReset)
            {
            ...
            if (switchPpq <= transportCursor + epsilon)
            {
                ...
                if (pulseOperationDue && pendingLoopBraceRange >= 0)
                    applyAudioLoopBraceRange (transportCursor);
        ...
        if (segmentEndsAtScheduledSwitch)
        {
            ...
            const auto pendingLoopBraceRangeAtSegmentEnd = pendingAudioLoopBraceRangePatternSlot;

            if (pulseOperationDue && pendingLoopBraceRangeAtSegmentEnd >= 0)
                applyAudioLoopBraceRange (segmentEnd);
            ...
            const auto otherPulseOpsDueAtSegmentEnd =
                ...
                && (nextPattern >= 0 || nextLoop >= 0 || pendingLoopBraceEnable >= 0
                    || pendingLoopBraceRangeAtSegmentEnd >= 0 || pendingPhaseResetAtSegmentEnd);
    ...
    for (int patternSlot = 0; patternSlot < patternSlotCount; ++patternSlot)
    {
        ...
        for (int row = 0; row < phraseRowCount; ++row)
        {
            ...
            rowTree.setProperty ("midiChannel",
                                 ...
                                 nullptr);
            rowTree.setProperty ("recordCycleLengthPulses",
                                 getPatternRecordCycleLengthPulses (patternSlot, row),
                                 nullptr);
...
void PluginProcessor::setStateInformation (const void* data, int sizeInBytes)
{
    ...
    for (int i = 0; i < state.getNumChildren(); ++i)
    {
        ...
        for (int rowIndex = 0; rowIndex < patternTree.getNumChildren(); ++rowIndex)
        {
            ...
            rebuildRowTimingLayout (steps);
            pattern.recordCycleLengthPulses[static_cast<size_t> (row)] =
                clampRecordCycleLengthPulses (
                    static_cast<int> (
                        rowTree.getProperty ("recordCycleLengthPulses",
                                             defaultRecordCycleLengthPulsesForRow (patternSlot, row))));
        }
    ...
    for (int i = 0; i < state.getNumChildren(); ++i)
    {
        ...
        modelSequencer().midiChannel[static_cast<size_t> (row)] = juce::jlimit (
            ...
            static_cast<int> (
                rowTree.getProperty ("midiChannel", defaultPhraseRowMidiChannelForRow (row))));
        rebuildRowTimingLayout (steps);
        modelPattern (getViewPatternSlot()).recordCycleLengthPulses[static_cast<size_t> (row)] =
            clampRecordCycleLengthPulses (
                static_cast<int> (
                    rowTree.getProperty ("recordCycleLengthPulses",
                                         defaultRecordCycleLengthPulsesForRow (
                                             getViewPatternSlot(),
                                             row))));
        publishRowToAudio (row);
