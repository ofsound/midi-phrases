...
TEST_CASE ("Cross-Mod mode can be enabled during playback without stopping transport", "[instance]")
{
    PluginProcessor testPlugin;
    ...
    testPlugin.processBlock (buffer, midi);
    CHECK (testPlugin.getPlaybackBeat() >= 0.0);
    ...
    CHECK (testPlugin.getPlaybackBeat() == Catch::Approx (1.5).margin (0.05));

    auto heardNoteOn = false;
