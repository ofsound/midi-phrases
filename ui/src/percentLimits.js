...
export const maxStepProbabilityValue = 100;
export const defaultStepProbabilityValue = maxStepProbabilityValue;
...
/** @param {number} value @param {{ fallback?: number }} [options] */
export function clampStepProbabilityPercent(value, { fallback = defaultStepProbabilityValue } = {}) {
  const parsed = Number.parseInt(String(value), 10);

  return Number.isNaN(parsed) ? fallback : Math.min(maxStepProbabilityValue, Math.max(0, parsed));
}
