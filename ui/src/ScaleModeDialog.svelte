...
<div class="fixed inset-0 z-[80] flex items-center justify-center bg-app/45 p-4 backdrop-blur-[14px] sm:p-6">
  ...
  <div
    ...
    tabindex="-1"
    class="relative z-[1] flex max-h-[min(94vh,56rem)] w-full max-w-[min(58rem,calc(100vw-2rem))] flex-col overflow-hidden border border-border/90 bg-app/88 shadow-[0_24px_80px_rgba(0,0,0,0.55)] backdrop-blur-[22px]"
  >
    <div class="flex items-start justify-between gap-4 border-b border-border-subtle px-5 py-4 sm:px-6">
      <div class="min-w-0">
        ...
        <h2 id={titleId} class="mt-1 text-2xl font-semibold text-text">Scale Mode</h2>
      </div>
      <button
        type="button"
        aria-label="Close scale mode"
        data-cursor="pointer"
        class="flex h-8 w-8 shrink-0 items-center justify-center border border-border bg-surface text-text-secondary outline-none transition-colors hover:border-border-strong hover:text-text focus:ring-1 focus:ring-focus-ring"
        onclick={onClose}
      >
        <svg
          class="h-4 w-4"
    ...
    <div class="grid shrink-0 gap-5 p-5 sm:p-6">
        ...
        <div class="grid grid-cols-6 gap-2 sm:grid-cols-12">
          {#each keyCenters as key (key.value)}
            <button
              ...
              data-cursor="pointer"
              class="flex h-11 items-center justify-center border text-sm font-semibold outline-none transition-colors focus:ring-1 focus:ring-focus-ring {root === key.value
      ...
      <div class="grid gap-3">
        <div>
          ...
          <h3 class="mt-1 text-lg font-semibold text-text">{scaleModes[modeIndex]?.label ?? "Chromatic"}</h3>
        </div>
