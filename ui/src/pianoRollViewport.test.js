import { describe, expect, it } from "vitest";
import {
  fittedPitchRangeForNotes,
  fittedPitchRangeForSchedule,
} from "./pianoRollViewport.js";

describe("fittedPitchRangeForSchedule", () => {
  it("uses a C3-C5 fallback for empty schedules", () => {
    expect(fittedPitchRangeForSchedule([])).toEqual({ minMidi: 48, maxMidi: 72 });
  });

  it("adds two semitones of padding and expands to at least five visible semitones", () => {
    expect(fittedPitchRangeForSchedule([{ midi: 60 }])).toEqual({ minMidi: 58, maxMidi: 62 });
  });

  it("uses the whole filtered schedule range without octave snapping", () => {
    expect(fittedPitchRangeForSchedule([{ midi: 57 }, { midi: 72 }])).toEqual({
      minMidi: 55,
      maxMidi: 74,
    });
  });

  it("clamps padding and minimum expansion to the MIDI note range", () => {
    expect(fittedPitchRangeForSchedule([{ midi: 0 }])).toEqual({ minMidi: 0, maxMidi: 4 });
    expect(fittedPitchRangeForSchedule([{ midi: 127 }])).toEqual({ minMidi: 123, maxMidi: 127 });
  });
});

describe("fittedPitchRangeForNotes", () => {
  it("matches the general piano-roll range for monophonic notes", () => {
    expect(fittedPitchRangeForNotes([])).toEqual({ minMidi: 48, maxMidi: 72 });
    expect(fittedPitchRangeForNotes([60])).toEqual({ minMidi: 58, maxMidi: 62 });
    expect(fittedPitchRangeForNotes([57, 72])).toEqual({ minMidi: 55, maxMidi: 74 });
  });
});
